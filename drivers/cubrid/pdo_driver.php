...
/**
 * CUBRID PDO database access classes
 *
 * @package objSQL
 * @version 3.9.0
...
class obj_connection extends obj_access
{
    ...
    protected function obj_db_connection()
    {
        ...
        if ( $obj_port )
			$obj_DSN = "cubrid:host={$this->obj_datasource[1]};port=$obj_port;dbname={$this->obj_datasource[4]}";
        else
            $obj_DSN = "cubrid:host={$this->obj_datasource[1]};dbname={$this->obj_datasource[4]}";
    ...
    {
        $obj_get_info = $this->obj_connection->query( "SELECT COLLATION('abc')" );
...
class obj_transaction
{
    ...
    public function obj_rollback( $savepoint=false )
    {
        if ( !$savepoint )
            ...
            $rollback = $this->obj_connection->query( "ROLLBACK WORK TO SAVEPOINT $savepoint" );
...
/* EOF pdo_driver.php */
/* Location: ./drivers/cubrid/pdo_driver.php */