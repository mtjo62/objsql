...
/**
 * MySQL/MariaDB PDO database access classes
 *
 * @package objSQL
 * @version 3.9.0
...
class obj_connection extends obj_access
{
    ...
    protected function obj_db_connection()
    {
        ...
        if ( !$obj_port )
			$obj_DSN = "mysql:host={$this->obj_datasource[1]};dbname={$this->obj_datasource[4]}";
		else
            $obj_DSN = "mysql:host={$this->obj_datasource[1]};port=$obj_port;dbname={$this->obj_datasource[4]}";
    ...
    {
		$obj_get_info = $this->obj_connection->query( "SHOW TABLE STATUS FROM {$this->obj_datasource[4]}" );

		return [$this->obj_connection->getAttribute( constant( 'PDO::ATTR_SERVER_VERSION' ) ), $obj_get_info->fetch( PDO::FETCH_NUM )[14], $this->obj_datasource[4]];
    }
...
/* EOF pdo_driver.php */
/* Location: ./drivers/mysql/pdo_driver.php */