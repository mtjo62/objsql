...
/**
 * SQL Server PDO database access classes
 *
 * @package objSQL
 * @version 3.9.0
...
class obj_connection extends obj_access
{
    ...
    protected function obj_db_connection()
    {
        ...
        if ( $obj_port )
        {
            $obj_DSN = "sqlsrv:Server={$this->obj_datasource[1]},$obj_port;Database={$this->obj_datasource[4]}";
        }
		else
        {
			$obj_DSN = "sqlsrv:Server={$this->obj_datasource[1]};Database={$this->obj_datasource[4]}";
        }
    ...
    {
		$obj_get_info = $this->obj_connection->query( "SELECT collation_name FROM sys.databases WHERE NAME='{$this->obj_datasource[4]}'" );

		return [$this->obj_connection->getAttribute( constant( 'PDO::ATTR_SERVER_VERSION' ) ), $obj_get_info->fetch( PDO::FETCH_OBJ )->collation_name, $this->obj_datasource[4]];
    }
...
class obj_prepare
{
   ...
   /**
    ...
    public function obj_free_statement()
    {
        ...
        return $this->obj_prepare_instance->closeCursor();
    }
...
class obj_transaction
{
    ...
    public function obj_rollback( $savepoint=false )
    {
        ...
        {
            $rollback = $this->obj_connection->query( "ROLLBACK TRANSACTION $savepoint" );
        }
    ...
    {
		return ( is_object( $this->obj_connection->query( "SAVE TRANSACTION $savepoint" ) ) ) ? true : false;
	}
}


/* EOF pdo_driver.php */
/* Location: ./drivers/sqlsrv/pdo_driver.php */