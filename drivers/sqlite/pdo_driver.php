...
/**
 * SQLite PDO database access classes
 *
 * @package objSQL
 * @version 3.9.0
...
class obj_connection extends obj_access
{
    ...
    {
        $obj_connection = new PDO( "sqlite:{$this->obj_datasource[4]}" );
    ...
    protected function obj_db_info()
    {
        ...
        return [$this->obj_connection->getAttribute( constant( 'PDO::ATTR_SERVER_VERSION' ) ), $query_stmt->fetch()['encoding'], $this->obj_datasource[4]];
    }
    ...
    protected function obj_db_rowcount( $table, $cols=false, $where=false )
    {
        ...
        {
            $row_count = $query_stmt->fetchColumn();
            $obj_return = ( $row_count >= 0 ) ? $row_count : -1;
...
class obj_resultset
{
   ...
   /**
    ...
    public function obj_num_rows()
    {
        ...
        while ( $this->obj_result->fetch( PDO::FETCH_NUM ) )
        {
            $num_rows++;
        }
...
class obj_transaction
{
    ...
    public function obj_rollback( $savepoint=false )
    {
        ...
        {
            $rollback = $this->obj_connection->query( "ROLLBACK TO $savepoint" );
        }
...
/* EOF pdo_driver.php */
/* Location: ./drivers/sqlite/pdo_driver.php */